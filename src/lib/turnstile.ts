...
export async function verifyTurnstile(
  ...
  if (!token) return { ok: false, status: 403, error: "Complete the security check before scanning." };
  ...
  try {
    ...
    if (!response.ok || !result.success || (result.action && result.action !== "identify")) {
